        ...
        <span key={i} className="flex items-center gap-1.5">
          ...
          {item.onClick ? (
            <button onClick={item.onClick} className="transition-colors hover:underline"
              style={{ color: i === items.length - 1 ? '#e8f4fd' : '#00c2ff' }}>
              {item.label}
            </button>
          ) : (
  ...
  const cards = [
    {
      ...
      description: 'Upload and manage AI knowledge base documents. Index PDFs, text files, and raw content for the chatbot.',
      badge: globalLoading ? null : `${docs.length} files`,
    },
    {
      id: 'sessions', icon: '💬', title: 'Sessions & Chats', color: '#00e676', glow: 'rgba(0,230,118,0.15)',
      description: 'Browse all chat sessions and drill into individual conversations. Each session contains a full chat history.',
      badge: globalLoading ? null : `${sessions.length} sessions`,
    },
    {
      id: 'leads', icon: '👤', title: 'Leads', color: '#a78bfa', glow: 'rgba(167,139,250,0.15)',
      description: 'View all captured leads from the chat widget. Name, email, and phone collected before the first message.',
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8">
        {[
          { label: 'Total Documents', value: globalLoading ? '…' : docs.length,    icon: '📄' },
          { label: 'Chat Sessions',   value: globalLoading ? '…' : sessions.length, icon: '🗂️' },
          { label: 'Total Leads',     value: globalLoading ? '…' : leads.length,   icon: '📋' },
    ...
    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-2">
          <div className="rounded-2xl p-5 sticky top-[88px]"
            ...
            <div className="flex rounded-xl overflow-hidden mb-4"
              ...
              {['file', 'text'].map(t => (
                <button key={t} onClick={() => setTab(t)} className="flex-1 py-2 text-xs font-semibold transition-all duration-200"
            ...
            <form onSubmit={handleUpload} className="flex flex-col gap-3">
              ...
              {tab === 'file' ? (
                <div onDragOver={e => { e.preventDefault(); setDragOver(true); }}
                  ...
                  className="rounded-xl p-6 flex flex-col items-center gap-2 cursor-pointer transition-all duration-200"
              ...
              <button type="submit" disabled={uploading}
                className="w-full py-3 rounded-xl font-bold text-sm transition-all hover:scale-[1.02] active:scale-95 disabled:opacity-50 flex items-center justify-center gap-2"
                ...
                {uploading ? (<><Spinner />Indexing…</>) : '⚡ Index Document'}
              </button>
            </form>
          </div>
        </div>
        ...
        <div className="lg:col-span-3">
          ...
          {loading ? (
            <div className="flex flex-col items-center justify-center py-24 gap-4"><Spinner /><p className="text-sm" style={{ color: '#7a9bb5' }}>Loading documents…</p></div>
          ) : docs.length === 0 ? (
...
function SessionsView({ sessions, loading, onSelectSession }) {
  ...
  const filtered = sessions.filter(s =>
    !search || s.session_name?.toLowerCase().includes(search.toLowerCase())
  );
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-5">
        <div className="flex-1 flex items-center gap-2 px-4 py-2.5 rounded-xl"
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search sessions…"
            ...
            style={{ color: '#e8f4fd', fontFamily: 'Outfit,sans-serif' }} />
        </div>
        {!loading && (
      ...
      {loading ? (
        ...
        <EmptyState icon="🗂️" title="No sessions found" subtitle={search ? 'Try a different search term' : 'No chat sessions have been created yet'} />
      ) : (
        <div className="flex flex-col gap-3">
          {filtered.map(session => (
            <button key={session.id} onClick={() => onSelectSession(session)}
              ...
              <div className="flex items-center gap-4">
                ...
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#7a9bb5" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"
...
/* ─────────────────── Session Detail View ─────────────────── */
// Chats are fetched here on demand (only when user clicks into a session)
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <EmptyState icon="📋" title="No leads found" subtitle={search ? 'Try a different search term' : 'No leads have been captured yet'} />
      ) : (
        <>
          ...
          <div className="flex flex-col gap-2">
            {filtered.map(lead => (
              <div key={lead.id} className="group rounded-xl transition-all duration-200"
...
/* ═══════════════════════════════════════════════
   Module-level cache — lives outside React's
   lifecycle. Survives StrictMode's double-mount
   so the fetch truly runs only once per page load.
═══════════════════════════════════════════════ */
let _cache = null;           // { docs, sessions, leads } once resolved
let _promise = null;         // in-flight promise, shared across mounts
...
export default function DocumentsPage() {
  ...
  const [loading,  setLoading]  = useState(!_cache); // skip spinner if cache hit

  const [view,            setView]           = useState('hub');
  const [selectedSession, setSelectedSession] = useState(null);

  useEffect(() => {
    // If already cached (e.g. StrictMode second mount), apply immediately
    if (_cache) {
      setDocs(_cache.docs);
  ...
  const breadcrumbs = {
    ...
    documents:       [{ label: 'Command Center', onClick: () => setView('hub') }, { label: 'Documents' }],
    sessions:        [{ label: 'Command Center', onClick: () => setView('hub') }, { label: 'Sessions & Chats' }],
    'session-detail':[
      { label: 'Command Center', onClick: () => setView('hub') },
      { label: 'Sessions & Chats', onClick: () => setView('sessions') },
      { label: selectedSession?.session_name || `Session #${selectedSession?.id}` },
    ],
    leads: [{ label: 'Command Center', onClick: () => setView('hub') }, { label: 'Leads' }],
  };
  ...
  return (
    <div className="min-h-screen" style={{ background: '#080f1a', fontFamily: 'Outfit,sans-serif', color: '#e8f4fd' }}>
      ...
      {/* Nav */}
      <header className="sticky top-0 z-30 flex items-center justify-between px-6 md:px-10 py-4"
        style={{ background: 'rgba(8,15,26,0.9)', borderBottom: '1px solid rgba(0,194,255,0.1)', backdropFilter: 'blur(20px)' }}>
        <div className="flex items-center gap-4">
          ...
          <img src="https://erpsolutions.oodles.io/wp-content/themes/ERP/custom_inc/image/home/erp_new_logo.png"
            alt="OodlesERP" className="h-8 object-contain" onError={e => e.target.style.display = 'none'} />
        </div>
        ...
        <div className="hidden md:flex items-center gap-1 rounded-xl p-1"
          ...
          {[
            ...
            <button key={tab.id}
              onClick={() => { setView(tab.id); setSelectedSession(null); }}
              className="px-3 py-1.5 rounded-lg text-xs font-semibold transition-all duration-200"
              style={{
                background: (view === tab.id || (view === 'session-detail' && tab.id === 'sessions')) ? 'rgba(0,194,255,0.15)' : 'transparent',
                color:      (view === tab.id || (view === 'session-detail' && tab.id === 'sessions')) ? '#00c2ff' : '#7a9bb5',
              }}>
              {tab.label}
            </button>
          ))}
      ...
      <main className="relative z-10 max-w-6xl mx-auto px-6 md:px-10 py-8">
        <div className="mb-6">
          ...
          <h1 className="text-2xl font-bold text-white mt-2">{viewTitles[view]}</h1>
        ...
        {view === 'hub' && (
          <HubView docs={docs} sessions={sessions} leads={leads} globalLoading={loading}
            onNavigate={v => { setSelectedSession(null); setView(v); }} />
        )}
        {view === 'documents' && (
          <DocumentsView docs={docs} loading={loading} onDocsChange={setDocs} />
        )}
        {view === 'sessions' && (
          <SessionsView sessions={sessions} loading={loading}
            onSelectSession={s => { setSelectedSession(s); setView('session-detail'); }} />
        )}
        {view === 'session-detail' && selectedSession && (
          <SessionDetailView session={selectedSession} />
        )}
        {view === 'leads' && (
          <LeadsView leads={leads} loading={loading} />
        )}
      </main>
